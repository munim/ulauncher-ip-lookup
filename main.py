...
class IPLookupExtension(Extension):
    ...
    def lookup(self, ip = None):
        params = {
            "apiKey": self.preferences['ipl_api_key'],
        }

        if ip != "?":
            params["ip"] = ip
        
        r = requests.get(ipgeolocation_api_url, params = params)
        ...
        if r.status_code == 200:
            ...
            out_arr["Location"] = "%s, %s (%s)" % (response.city, response.country_name, response.country_code2)
            out_arr["Coordiates"] = "%s, %s" % (response.latitude, response.longitude)
            out_arr["Calling code"] = response.calling_code
            out_arr["Languages"] = response.languages
            out_arr["ISP"] = response.isp
            out_arr["Currency"] = "%s (%s)" % (response.currency.symbol, response.currency.code)
